...
class ServiceDocumentView(ContentNegotiatedMethodView):
    ...
    def post(self):
        content_disposition, content_disposition_options = parse_options_header(request.headers.get('Content-Disposition', ''))
        ...
        packaging = request.headers.get('Packaging', current_app.config['SWORD_DEFAULT_PACKAGING_FORMAT'])
        ...
        packaging.ingest(record=record, stream=request.stream, filename=filename, content_type=content_type)
